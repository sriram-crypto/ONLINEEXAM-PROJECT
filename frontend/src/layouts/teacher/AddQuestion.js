...
const AddQuestion = () => {
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch('/api/teacher/addquestion/course-categories');
  ...
  const fetchCourses = async (categoryId) => {
    try {
      const response = await fetch(`/api/teacher/addquestion/courses?category_id=${categoryId}`);
      const data = await response.json();
      setCourses(data);
    } catch (error) {
  ...
  const fetchSubjects = async (courseId) => {
    try {
      const response = await fetch(`/api/teacher/addquestion/subjects?course_id=${courseId}`);
      const data = await response.json();
      setSubjects(data);
    } catch (error) {
  ...
  const fetchChapters = async (subjectId) => {
    try {
      const response = await fetch(`/api/teacher/addquestion/chapters?subject_id=${subjectId}`);
  ...
  const fetchLevels = async () => {
    try {
      const response = await fetch('/api/teacher/addquestion/difficulty-levels');
      const data = await response.json();
      setLevels(data);
    } catch (error) {
  ...
  const fetchQuestionTypes = async () => {
    try {
      const response = await fetch('/api/teacher/addquestion/question-types');
      const data = await response.json();
  ...
  const handleExcelFormChange = (e) => {
    ...
    if (name === 'category') {
      setExcelForm(prev => ({ ...prev, course: '', subject: '', chapter: '' }));
      setCourses([]);
      setSubjects([]);
      setChapters([]);
      if (value) fetchCourses(value);
    } else if (name === 'course') {
      setExcelForm(prev => ({ ...prev, subject: '', chapter: '' }));
      setSubjects([]);
      setChapters([]);
      if (value) fetchSubjects(value);
    } else if (name === 'subject') {
      setExcelForm(prev => ({ ...prev, chapter: '' }));
      setChapters([]);
  ...
  const handleManualFormChange = (e) => {
    ...
    if (name === 'category') {
      setManualForm(prev => ({ ...prev, course: '', subject: '', chapter: '' }));
      setCourses([]);
      setSubjects([]);
      setChapters([]);
      if (value) fetchCourses(value);
    } else if (name === 'course') {
      setManualForm(prev => ({ ...prev, subject: '', chapter: '' }));
      setSubjects([]);
      setChapters([]);
      if (value) fetchSubjects(value);
    } else if (name === 'subject') {
      setManualForm(prev => ({ ...prev, chapter: '' }));
      setChapters([]);
      if (value) fetchChapters(value);
    }
  };
  ...
  const handleImageUpload = async (e, fieldName) => {
    ...
    try {
      ...
      const response = await fetch('/api/teacher/addquestion/upload-image', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleExcelUpload = async (e) => {
    ...
    try {
      ...
      const response = await fetch('/api/teacher/addquestion/upload-questions', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleManualSubmit = async (e) => {
    ...
    if (!manualForm.category || !manualForm.course || !manualForm.subject || !manualForm.level_id || !manualForm.type || !manualForm.question_text) {
      setError('Please fill all required fields');
      return;
    }
 ...
 */
    ...
    try {
      ...
      const response = await fetch('/api/teacher/addquestion/add-manual-question', {
        method: 'POST',
        headers: {
  ...
  const handleDownloadFormat = async (type = 'basic') => {
    const url = `/api/teacher/addquestion/download-format?type=${type}`;
    try {
  ...
  return (
    <Box className="teacher-tool teacher-tool-questions" sx={{ p: 0 }}>
      <section className="teacher-tool-hero">
        <div>
          ...
          <h2>Add questions by Excel upload or manual entry.</h2>
          <p>
            Keep question creation, media uploads, templates, and the searchable question table in
            the same teaching workspace.
          </p>
        </div>
        <div className="teacher-tool-actions">
          <button type="button" className="teacher-action-button primary" onClick={() => setTabValue(1)}>
            <span className="material-icons-round">edit</span>
            Manual Entry
          </button>
          <button type="button" className="teacher-action-button secondary" onClick={() => handleDownloadFormat('basic')}>
      ...
      <div className="teacher-tool-stat-strip">
        <article>
          <strong>{tabValue === 0 ? "Excel" : "Manual"}</strong>
        ...
        <article>
          <strong>{questionTypes.length}</strong>
          <span>Question types</span>
        </article>
      </div>

      <Paper className="teacher-tool-card" elevation={0} sx={{ p: { xs: 2, sm: 3 } }}>
        <Box className="teacher-section-heading">
          <span className="teacher-tool-kicker">Question editor</span>
          <h2>Add Questions</h2>
          <p>Upload a formatted spreadsheet or create a question with text, options, and images.</p>
        </Box>
      ...
      <Box className="teacher-table-shell">
        <div className="teacher-table-title">
          <div>
            <span className="teacher-tool-kicker">Question records</span>
            <h3>All Questions</h3>
          </div>
        </div>
        <Box sx={{ p: { xs: 2, sm: 3 } }}>

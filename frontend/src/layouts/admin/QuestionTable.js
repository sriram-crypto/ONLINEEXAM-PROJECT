...
function QuestionTable({ reloadFlag }) {
  ...
  // Reset subject and chapter when course changes, and fetch subjects for the selected course
  useEffect(() => {
    setSubjectFilter('');
    setChapterFilter('');
    setChaptersForSubject([]);
    ...
    if (courseFilter) {
      (async () => {
        try {
          const res = await fetch(`/api/admin/subjects?course_id=${courseFilter}`);
          const data = await res.json();
          setAllSubjects(Array.isArray(data) ? data : []);
        } catch (e) {
          console.error('Error fetching subjects for course:', e);
        }
      })();
    } else {
      // If no course selected, fetch all subjects
      (async () => {
        try {
        ...
        {hasImage ? (
          <Box>
            <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2, mb: 2 }}>
              <Box sx={{ cursor: 'pointer' }} onClick={() => window.open(`${process.env.REACT_APP_IMAGE_BASE_URL}${editRow[fieldName]}`, '_blank')}>
                <img
                  src={`${process.env.REACT_APP_IMAGE_BASE_URL}${editRow[fieldName]}`}
                  alt={label}
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) =>
        params.row.question_image ? (
          (() => {
            let imgPath = params.row.question_image;
            // Remove any leading /uploads/images/ to prevent double
            while (imgPath.startsWith('/uploads/images/')) {
              imgPath = imgPath.replace('/uploads/images/', '');
            }
            // Now construct the final URL
            imgPath = `${process.env.REACT_APP_IMAGE_BASE_URL}${imgPath}`;
            return (
              <img
                src={imgPath}
                alt="Question"
    ...
    },
    {
      ...
      renderCell: (p) =>
        p.row.option1_image ? (
          <img
            src={`${process.env.REACT_APP_ANS_IMAGE_BASE_URL}${p.row.option1_image}`}
            alt="Option 1"
            style={{ width: '30px', height: '25px', objectFit: 'cover', cursor: 'pointer' }}
            onClick={() => window.open(`${process.env.REACT_APP_ANS_IMAGE_BASE_URL}${p.row.option1_image}`, '_blank')}
            title="Click to view full size"
          />
        ) : '-'
    },
    ...
    },
    {
      ...
      renderCell: (p) =>
        p.row.option2_image ? (
          <img
            src={`${process.env.REACT_APP_ANS_IMAGE_BASE_URL}${p.row.option2_image}`}
            alt="Option 2"
            style={{ width: '30px', height: '25px', objectFit: 'cover', cursor: 'pointer' }}
            onClick={() => window.open(`${process.env.REACT_APP_ANS_IMAGE_BASE_URL}${p.row.option2_image}`, '_blank')}
            title="Click to view full size"
          />
        ) : '-'
    },
    ...
    },
    {
      ...
      renderCell: (p) =>
        p.row.option3_image ? (
          <img
            src={`${process.env.REACT_APP_ANS_IMAGE_BASE_URL}${p.row.option3_image}`}
            alt="Option 3"
            style={{ width: '30px', height: '25px', objectFit: 'cover', cursor: 'pointer' }}
            onClick={() => window.open(`${process.env.REACT_APP_ANS_IMAGE_BASE_URL}${p.row.option3_image}`, '_blank')}
            title="Click to view full size"
          />
        ) : '-'
    },
    ...
    },
    {
      ...
      renderCell: (p) =>
        p.row.option4_image ? (
          <img
            src={`${process.env.REACT_APP_ANS_IMAGE_BASE_URL}${p.row.option4_image}`}
            alt="Option 4"
            style={{ width: '30px', height: '25px', objectFit: 'cover', cursor: 'pointer' }}
            onClick={() => window.open(`${process.env.REACT_APP_ANS_IMAGE_BASE_URL}${p.row.option4_image}`, '_blank')}
            title="Click to view full size"
          />
        ) : '-'
    },
    ...
    <>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 16, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <FormControl style={{ minWidth: 180 }} size="small">
          <CustomDropdown
            label="Subject"
            options={[{ value: '', label: 'All' }, ...allSubjects.map(s => ({ value: s.subject_id, label: s.subject_name }))]}
            value={subjectFilter}

...
const fileFilter = (req, file, cb) => {
  ...
  if (file.fieldname === "file") {
    ...
    if (
      ...
      cb(
        new Error(
          "Only Excel files (.xlsx, .xls) are allowed for question upload!"
        ),
        false
      );
    }
...
// GET /api/admin/chapters - get chapters for a subject (for dropdown)
router.get("/chapters", (req, res) => {
  ...
  db.query(
    "SELECT chapter_id, chapter_name, description FROM chapters WHERE subject_id = ? ORDER BY chapter_name",
    [subject_id],
    (err, results) => {
      if (err)
...
router.get("/clear-all-questions", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "All questions cleared from the database.",
    });
  } catch (error) {
...
// GET /api/admin/categories-all - get all categories for dropdown
...
// GET /api/admin/subjects-all - get all subjects for dropdown with course_id
router.get("/subjects-all", (req, res) => {
  db.query(
    `SELECT s.subject_id, s.subject_name, csm.course_id
     FROM subjects s
     LEFT JOIN course_subject_mapping csm ON s.subject_id = csm.subject_id
...
router.get("/questions", (req, res) => {
  const sql = `
    SELECT qa.*, s.subject_name, c.course_name, qt.type_name
  ...
  db.query(sql, (err, results) => {
    if (err) return res.status(500).json({ error: err.message });
    // DataGrid expects an 'id' field and 'type' field for question type name
    res.json(results.map((q) => ({ 
      ...q, 
      id: q.id,
      type: q.type_name // Add the question type name as 'type' field
    })));
  });
});

// GET /api/admin/questions/:id - get a single question by ID
...
router.delete("/questions/:id", (req, res) => {
  const { id } = req.params;
  db.query(
    "DELETE FROM questions_answers WHERE id = ?",
    [id],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
...
router.put("/questions/:id", (req, res) => {
  ...
  db.query(
    `UPDATE questions_answers SET course_id=?, subject_id=?, level_id=?, question_type_id=?, question_text=?, option1=?, option2=?, option3=?, option4=?, answer=?, chapter_id=?, class=?, schoolname=?, question_image=?, option1_image=?, option2_image=?, option3_image=?, option4_image=?, answer_image=? WHERE id=?`,
    [
      course_id,
      subject_id,
      level_id,
...
router.get("/question-types", (req, res) => {
  console.log("GET /admin/question-types called");
  db.query(
    "SELECT question_type_id, type_name FROM question_types ORDER BY question_type_id",
    (err, results) => {
      if (err) {
        console.error("DB error in /admin/question-types:", err);
        return res.status(500).json({ error: err.message });
      }
...
router.get("/courses", (req, res) => {
  ...
  db.query(
    "SELECT course_id, course_name FROM courses WHERE category_id = ?",
    [category_id],
    (err, results) => {
      if (err) {
...
router.get("/activate-or-deactivate/users", (req, res) => {
  ...
  let sql =
    "SELECT user_id, name, email, role, is_active, created_at FROM users WHERE 1=1";
  const params = [];
  if (role) {
    sql += " AND role = ?";
    params.push(role);
...
router.post("/setup-question-types", (req, res) => {
  ...
  const questionTypes = [
    ...
    [
      5,
      "Fill in the Blanks",
      "Blank(s) to be filled with the correct word or value",
    ],
  ...
  db.query("DELETE FROM question_types", (err) => {
    ...
    db.query(sql, [questionTypes], (err, result) => {
      ...
      console.log(
        `Successfully inserted ${result.affectedRows} question types`
      );
      res.json({
        success: true,
...
router.post(
  ...
  async (req, res) => {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        error:
          "Missing required fields: course, subject, level_id, type, question_text, answer",
      });
    }
    ...
    try {
      ...
      const questionTypeResult = await new Promise((resolve, reject) => {
        db.query(
          "SELECT question_type_id FROM question_types WHERE LOWER(type_name) = LOWER(?)",
          [type.trim()],
          (err, results) => {
            if (err) reject(err);
      ...
      if (req.files) {
        if (req.files.question_image)
          questionImage = `/uploads/images/${req.files.question_image[0].filename}`;
        if (req.files.option1_image)
          option1Image = `/uploads/images/${req.files.option1_image[0].filename}`;
        if (req.files.option2_image)
          option2Image = `/uploads/images/${req.files.option2_image[0].filename}`;
        if (req.files.option3_image)
          option3Image = `/uploads/images/${req.files.option3_image[0].filename}`;
        if (req.files.option4_image)
          option4Image = `/uploads/images/${req.files.option4_image[0].filename}`;
      }
  ...
  (category_id, course_id, subject_id, chapter_id, level_id, question_type_id,
   ...
   answer, class, schoolname, created_at,
   question_image, option1_image, option2_image, option3_image, option4_image, answer_image)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
...
router.post("/upload-questions", upload.single("file"), async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      const { category_id, course_id, subject_id, chapter_id, level_id } =
        req.body;

      // Validate required fields for Excel upload (no type required)
      if (!category_id || !course_id || !subject_id || !level_id) {
        console.log("Missing required fields for Excel upload:", {
          category_id,
          course_id,
          subject_id,
          level_id,
        ...
        return res
          ...
          .json({
            success: false,
            error: "Please provide all mandatory fields for Excel upload.",
          });
      }
      ...
      try {
        const typeResult = await new Promise((resolve, reject) => {
          db.query(
            "SELECT question_type_id FROM question_types ORDER BY question_type_id LIMIT 1",
            (err, results) => {
              if (err) reject(err);
              else resolve(results);
            }
      ...
      if (rows.length === 0) {
        return res
          ...
          .json({
            success: false,
            error: "No valid questions found in the Excel file.",
          });
      }
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (!row["option1"] || !row["option2"]) {
          failedRows.push(
            `Question ${i + 1}: Questions must have at least 2 options`
          );
          continue;
        }
        ...
        try {
          const sql = `
            INSERT INTO questions_answers (
              category_id, course_id, subject_id, chapter_id, level_id, question_type_id, question_text, option1, option2, option3, option4, answer, class, schoolname, created_at,question_image, option1_image, option2_image, option3_image, option4_image, answer_image
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `;
          ...
          failedRows.push(
            `Question: ${questionData.question_text} - ${error.message}`
          );
        }
      }
    ...
    res
      ...
      .json({
        success: false,
        error: "Error uploading questions: " + error.message,
      });
  }
});


// POST /api/admin/upload-image - upload single image for admin
router.post("/upload-image", uploadImages.single("image"), (req, res) => {
  try {
    if (!req.file) {
      return res
...
// POST /api/teacher/addquestion/upload-image - upload single image (for backward compatibility)
router.post("/teacher/addquestion/upload-image", uploadImages.single("image"), (req, res) => {
  try {
    if (!req.file) {
      return res
        .status(400)
...
router.get("/download-format", (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Error generating format file" });
  }
});
...
router.post("/add-manual-question", async (req, res) => {
  try {
    ...
    if (
      requiredFields.some(
        (f) => f === undefined || f === null || f === "" || isNaN(parseInt(f))
      ) ||
      ...
      return res.status(400).json({
        success: false,
        error:
          "Please provide all required fields: category_id, course_id, subject_id, level_id, question_type_id, and either question_text or question_image",
      });
    }
    ...
    const getQuestionType = () => {
      return new Promise((resolve, reject) => {
        db.query(
          "SELECT type_name FROM question_types WHERE question_type_id = ?",
          [question_type_id],
          (err, results) => {
            if (err) reject(err);
            else resolve(results);
    ...
    /* if (
      (questionTypeName === "MCQ" || questionTypeName === "MSQ") &&
...
`;
      ...
      return new Promise((resolve, reject) => {
        db.query(
          ...
          [
            ...
            data.class,
            data.schoolname,
            data.created_at, // correctly placed
            data.question_image,
            data.option1_image,
            data.option2_image,
            data.option3_image,
    ...
    const questionData = {
      ...
      created_at: now1, // MUST come here before images

...
// Update exam details
router.put('/exams/:exam_id', (req, res) => {
  const { exam_id } = req.params;
  const { title, duration, status, exam_date, start_time, end_time } = req.body;
  ...
  if (start_time !== undefined) {
    ...
    values.push(combineDateTime(exam_date || '', start_time));
  }
  if (end_time !== undefined) {
    updates.push('end_time = ?');
    values.push(combineDateTime(exam_date || '', end_time));
  }
  ...
  values.push(exam_id);
  const query = `UPDATE exams SET ${updates.join(', ')} WHERE exam_id = ?`;
...
router.get('/filter', (req, res) => {
  try {
    ...
    db.query(sql, params, (err, results = []) => {
      ...
      const normalized = results.map((r, index) => ({
        id: r.id,
...
router.post('/preview-audience', (req, res) => {
  ...
  const buildFilterSQL = (filterObj, operatorAND = true) => {
    ...
    if (filterObj.email) { conditions.push('LOWER(u.email) LIKE LOWER(?)'); params.push(`%${filterObj.email}%`); }
  ...
  try {
    ...
    try {
      ...
      const examInsert = await queryAsync(
        ...
        [title, Number(course_id), Number(duration), exam_date, startDateTime, endDateTime, Number(created_by), status, examPackage || null, schoolname, (!classNo || isNaN(Number(classNo))) ? null : Number(classNo), orderType || null]
      );
      ...
      const qRows = [];
      console.log('Processing subjects for exam_id:', exam_id);
      console.log('Subjects array:', JSON.stringify(subjects, null, 2));
      subjects.forEach((s, sectionIdx) => {
        console.log(`Subject ${sectionIdx}: ${s.subject_id}, selectedQuestions:`, s.selectedQuestions);
      ...
      console.log('Question rows to insert:', qRows.length, qRows);
      if (qRows.length) {
        await queryAsync(
          'INSERT INTO exam_question_mapping (exam_id, question_id, section, order_no, marks, negative_marks) VALUES ?',
          [qRows]
        );
        console.log('Successfully inserted', qRows.length, 'questions into exam_question_mapping');
      } else {
        console.warn('WARNING: No questions to insert! qRows is empty');
      }
